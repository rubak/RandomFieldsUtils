...
utilsoption_type * RFU_GLOBAL_OPTIONS = &OPTIONS;
...
void setoptions(int i, int j, SEXP el, char name[LEN_OPTIONNAME], 
  ...
  case 0: {// general
	...
	if (neu == LA_QUERY) {
	  if (options == RFU_GLOBAL_OPTIONS) // OK
...
void utilsoption_NULL (utilsoption_type *S) {
  ...
  MEMCOPY(S, RFU_GLOBAL_OPTIONS, sizeof(utilsoption_type)); // OK
...
void SetLaMode(la_modes usr_mode) {
  ...
  RFU_GLOBAL_OPTIONS->solve.tinysize =
    RFU_GLOBAL_OPTIONS->basic.LaMaxTakeIntern = -1;
#define TINY_SIZE_MAX 3  
  if (la_mode == LA_INTERN) {
    RFU_GLOBAL_OPTIONS->solve.tinysize = TINY_SIZE_MAX;
    RFU_GLOBAL_OPTIONS->basic.LaMaxTakeIntern = MAXINT;
...
#  ifdef SCHLATHERS_MACHINE
    ...
    PL = 0;
#  endif  
    RFU_GLOBAL_OPTIONS->basic.LaMaxTakeIntern = own_chol_up_to();
    RFU_GLOBAL_OPTIONS->solve.tinysize = MIN(TINY_SIZE_MAX, RFU_GLOBAL_OPTIONS->basic.LaMaxTakeIntern);
    if (PL > 0)
      PRINTF("Limit size for facile Cholesky algorithm  = %d\n",
	     RFU_GLOBAL_OPTIONS->basic.LaMaxTakeIntern);
#  ifdef SCHLATHERS_MACHINE
#else   
    PL = PLalt;
#  endif
  ...
  if ((la_mode == LA_GPU || la_mode == LA_R) &&
      RFU_GLOBAL_OPTIONS->solve.pivot_mode > PIVOT_AUTO)
    ERR("Pivotized Cholesky decomposition has not been implemented yet for GPU and the LAPACK library");
  
  RFU_GLOBAL_OPTIONS->basic.la_mode = la_mode;
}
void SetLaMode() {
   SetLaMode(RFU_GLOBAL_OPTIONS->basic.la_usr);
}
